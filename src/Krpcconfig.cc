...
KrpcConfig &KrpcConfig::Instance()
{
    static KrpcConfig config;
    return config;
}

void KrpcConfig::InitEnv(int argc, char **argv)
{
    if (argc < 2) {
    ...
    KrpcConfig::Instance().LoadConfigFile(config_file.c_str());
...
std::optional<std::string> KrpcConfig::Lookup(const std::string &key)
{
