...
void KrpcProvider::RegisterService(google::protobuf::Service *service)
{
...
{
    // 读取配置文件rpcserver的信息
    std::string ip = KrpcConfig::Instance().Lookup("rpcserver_ip").value_or("127.0.0.1");
    std::string port = KrpcConfig::Instance().Lookup("rpcserver_port").value_or("8000");

    // 使用muduo网络库，创建address对象
    muduo::net::InetAddress address(ip, std::atoi(port.c_str()));

    // 创建tcpserver对象
    std::unique_ptr<muduo::net::TcpServer> server = std::make_unique<muduo::net::TcpServer>(&m_event_loop, address, "KrpcProvider");
...
void KrpcProvider::onMessage(const muduo::net::TcpConnectionPtr &conn, muduo::net::Buffer *buffer, muduo::Timestamp receive_time)
{
    ...
    ***/
    google::protobuf::Closure *done = google::protobuf::NewCallback<KrpcProvider, const muduo::net::TcpConnectionPtr &, google::protobuf::Message *>(this, &KrpcProvider::sendRpcResponse, conn, response);
